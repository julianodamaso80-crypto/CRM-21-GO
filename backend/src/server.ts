...
async function bootstrap() {
  try {
    ...
    // CRM em /crm (registrar PRIMEIRO para que o prefix /crm tenha prioridade)
    if (crmDistPath) {
      ...
      await fastify.register(fastifyStatic, {
        ...
        prefix: '/crm/',
        wildcard: false,
        decorateReply: !siteDistPath, // se site tambem registrar, evita conflito
      })
    }
    ...
    // Not-found handler: roteia para SPA ou site conforme o path
    fastify.setNotFoundHandler((request, reply) => {
      // API e rotas especiais: 404 JSON
    ...
    if (!siteDistPath) {
      console.log('[Static] Site dist not found, looked in:', sitePaths)
    }
    if (!crmDistPath) {
      console.log('[Static] CRM dist not found, looked in:', crmPaths)
    }
